...
def build_tools(
    ...
    async def ask_user(questions: list[AskUserQuestion]) -> str:
        ...
        1) 这一轮【动手写代码前】（常规）：满足下面任一条件就该在第一次调用
           ...
           调用问清楚；没有依赖关系的问题不要拆成多次调用：
           a) 存在会显著影响这一轮走向、且没有合理默认值的关键分歧（如整体风格该走
              极简还是国潮）。页面组织方式不属于这类分歧：禁止询问“单页还是多路由”，
              应根据内容职责自行判断；从零生成内容型网站且具体模块未说明时，只用普通
              多选询问用户想展示哪些内容，选项避免“路由、锚点、信息架构”等技术术语；
              博客、文档、商城、列表 + 详情、需要独立分享地址或持续增长的内容应自行
              使用多路由，不能默认把所有内容压成一个长页面；
              用户明确说“新增 XX 页面 / 详情页 / 列表页”时没有分歧，应直接创建
              独立路由，不能把“页面”改写成当前页的 section 或模块；
           b) 用户明确要求新增一批内容，但没有说明具体交付范围，且不同选择会直接改变
              要创建的页面或能力（如只说"再加几个页面"、"加国际化"却没说页面或
              语言）。从零生成本身不是提问理由；只要用户已经说明交付类型以及至少一个
              核心页面、内容或功能，就应采用合理默认直接开工。比如“做一个带作品展示
              和联系表单的个人主页”已经足够明确，禁止再问深色模式、标签分类、社交
              链接或个人简介等锦上添花选项；能形成完整可用首版时，不要调用本工具。
        ...
        - multi=true（多选）：用于补齐会实质改变交付范围的一批内容项（如要新增哪些
          页面、国际化要覆盖哪些语言），options 需要 1~6 个独立选项，用户可以
          一个都不勾，这是合法结果。
        前端会为每个问题额外提供一个自定义文字回答的入口，你不必关心这件事——只要
        正常处理返回的汇总文本即可，它可能是选项原文，也可能是用户自己写的话。
