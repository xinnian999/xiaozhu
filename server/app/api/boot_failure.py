"""Boot failure API —— 接收前端「预览运行环境启动失败」的上报。

WebContainer 的运行时需从境外 boot，国内偶发失败。前端 boot 超时/抛错时把原因 + 环境
信息 POST 到这里，落 boot_failures 表，管理后台据此监控失败率、定位偶发原因。
...
async def report_boot_failure(
    ...
    """记录一次 boot 失败。user_id 从登录态取，UA 从请求头取，其余由前端上报。"""
