...
// ============================================
// 与主前端 web/src/lib/lobeIcon.ts 一致：模型的 logo 字段是 @lobehub/icons 的
// 「组件标识符」（不是 URL），格式 "{Name}" 或 "{Name}.{Variant}"，
// 如 "OpenAI" / "Claude.Color" / "Qwen.Color"。整库很大，改成动态 import 打成独立
// chunk，避免拖慢首屏；模块级缓存 Promise，整个会话只下载一次。
...
function resolve(
  ...
  if (Comp == null || (typeof Comp !== 'function' && typeof Comp !== 'object')) return null
