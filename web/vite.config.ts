...
// 后端 dev server 地址。前台经 vite(7000) 代理到它，做到「开发期只用一个端口」。
...
// 保持 Host 头为浏览器原始的 localhost:7000，让后端生成的绝对地址继续走 Vite 代理。
const backendProxy = {
  target: BACKEND,
  changeOrigin: false,
};
...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      // 这样开发期只用 vite 一个端口(7000)就能同时访问前台和后台，不必记两个端口。
